...
## 附 6：kw-13 终局（2026-09-04，C10 分析三轮：race 闭合但数值不动，双永久遗留定案）

- **分析三件套**：①快照 v2（MTE2 有序读修好 kw-12 全零）发现 C2V1/C2V2 循环旗标被软流水
  错位一轮，且各循环滞后可见状态全干净 → 污染窗口收窄到最后循环+V2 读；②双读判别当场捕获
  GM 窗口内 +67 变化；③C2V2Last 任务级握手闭合 race（early==late）
- **关键复判**：race 闭合后 C10 mere 0.9538 逐位不变；修复前捕获的 +67 是 V2 读后的良性晚写
  （从未被消费）→ **mm2ResGm 终值本身即错**（V2 捕获值即错）
- **嫌疑终局收窄**：fixpipe 原子写在 m<64 几何下写出错误终值（kw-10~13 全部去混淆后唯一剩余）
- **下一战役留档**（§P6.8）：dump 脏行完整 512 列累计器终值 + 除数，与宿主 fp64 模拟
  n(i) 量化尺度链逐列对照，定位错误数值的代数形态
- C2V2Last 保留为 hardening；MLA 15/16 ulp + GQA 零回退维持（终二进制复核）
- 提交：26001f38b（快照 v2）/ 35c7bbd6f（双读探针）/ 9faf61a2b（C2V2Last）
