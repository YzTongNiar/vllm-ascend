...
## P6.7 kw-12 轮（2026-09-04，卡 4）：C02 攻坚——协调者假设证伪 + 三大写入者排除，C02/C10 并列永久遗留（硬停止）

### 假设证伪（免构建定性）

C02 逐 batch 尾宽梯度：**全部 8 个 batch 脏——含 b0（kv=4096=整 8 块、尾宽 0、无膨胀
无尾块）**；lse 全对（4e-6）；纯正向膨胀（ratio>1 仅）。→ **"sm0 尾宽膨胀漏网"假设
不成立：C02 即 C10 同族缺陷**（多核并发 + m<128 类），与 mask/尾宽无关。

### A/B 探针（构建 1，metadata[603] 魔数开关跳过 nupdate 原子拷贝，单二进制对照）

| case | nupdate=ON | nupdate=SKIP |
|---|---|---|
| C02 | 0.86-0.99 | 1.99（恶化） |
| C10 | 0.95 | 1.48（恶化） |
| C05/C08 | 0.0078 | **2.06/2.01（崩）** |
| C12 | 0.0078 | 0.81（崩） |

→ **nupdate 数值承重且正确（收敛 case 依赖它）；nupdate 原子链排除为污染源**。

### 去混淆实验（免构建，B=2/G=16/H=8 → m=128 但 FD 组恰 1/核无拆分）

m128-1组/核 sm0 与 sm3 **全净**；m=32 1组/核脏；m=56（C02 形）脏 → **FD-combine
无拆分路径排除**；缺陷唯一稳定轴确认为 **actMBaseSize ∈ (56,64] 边界以下 + 并发核
≥16 + 每 split ≥2 循环**（自然 m=64/128 净；kw-10b/b2 的 cube 侧垫 m 不等效——提示
污染位点在 cube 外的 m 观测面：V2 读路径或跨循环原子链的 m 相关几何）。

### 逐循环快照探针（构建 2，metadata[604/605]）——受流水线偏移污染（读早，全零），弃用

### 排除总表（C10/C02 公共缺陷，历经 kw-10b/b2 + kw-11 + kw-12）

mm2 cube M 链（垫 64/128）、fixpipe mSize（垫 128）、L0C M/FIX 握手（恢复）、
slot 跨任务复用（V2C2 反向边 + 1任务/核仍脏）、nupdate 原子链（A/B）、FD-combine
无拆分（去混淆）、FD staging（kw-11 探针：污染在累计器读点）、除数（D 均匀）、
尾块几何（b0 无尾脏）、mm1（lse 干净）、vecDealM 轴（m56/m64 同 32 一脏一净）。

### 剩余嫌疑（下一战役起点）

V2 读路径（DealBmm2ResBaseBlockAmla 的 DataCopy/pingpong 链在 dealRowCount<32 时的
行为）或同任务跨循环 fixpipe 原子链的 m<128 几何。建议探针：**逐元素累计器 dump**
（在 V2 读后对脏行整行 512 元素 dump，与宿主重算的期望部分和逐列对照）——kw-12 的
[603]/[604] 惰性基建常驻可扩展。

### 终态（kw-12 二进制 = kw-9a/9b + kw-10 + bt 钳位 + kw-11 三硬ening + kw-12 两惰性探针）

- MLA C04-C20：**15/16 ulp 确定性**（唯 C10）；C02（表外新测）与 C10 并列永久遗留
- GQA：G1/G2 bit=True mere=0.0、C21 6.3e-6——与 kw-4 终态逐位同值
- C08 性能补测：**0.274×**（快于官方 3.6×）
- kw-12 用 2/3 轮构建（第 3 轮无可辩护候选，按单变量纪律不盲花）


## P6.8 kw-13 轮（2026-09-04，卡 4）：分析先行三轮——旗标滞后/双读基建/C2V2Last 硬ening，
C10/C02 维持双永久遗留（硬停止，3/3 轮用尽）

### 分析轮成果（无实锤级修法命中，但三项新知识入库）

1. **快照 v2（构建 1）**：kw-12 全零根因＝标量 GM 读走 S 队列、不被 MTE2 队列的 C2V1
   等待约束；改 MTE2 DataCopy + 事件后出真数据。**发现一循环旗标滞后**：V1 循环 L 时点
   累计器仅含 [0..L-1]（loop2 快照比值与 loop1 期望逐位一致）——C2V1/C2V2 按循环配对
   被软流水错位一轮。滞后可见的各循环累计状态全部干净（c2/c3 无持续污染）。
2. **双读判别（构建 2）**：V2 首读采样 vs RowDivs 后重读。修复前 odd AIV r1 出现
   early=57 → late=124（+67 在 V2 窗口内落地）——一度判为"末循环写追上 V2 读"。
3. **C2V2Last（构建 3，保留为硬ening）**：按任务 1:1 严格配对的"末循环 fixpipe 完成"
   旗标（AIC 每任务末循环 fixpipe 后双 bank 置位 / V2 每任务唯一读前单等待；与 C2V1
   通道同构）。**修复后双读 early==late（race 闭合）但 C10 mere 0.9538 逐位不变**——
   复判修正解读：V2 的捕获值（=early 采样）在两版构建中同为 57，修复前的 +67 是
   **V2 读之后的良性晚写**（production 从未消费；slot 被 T+2 首循环覆盖写冲掉）。
   C2V2Last 仍保留：语义正确的排序保证（防其它时序下 V2 早读）。

### C10/C02 终局状态（双永久遗留，证据链完整）

- 终值：C10 mere 0.9538、C02 mere 0.86-0.99（多轮稳定）
- 缺陷轴（去混淆后唯一稳定）：actMBaseSize < ~64 + 并发核 ≥16 + 每 split ≥2 循环
- 污染定位（探针）：在 mm2ResGm 的终值本身（V2 捕获值即错），非读时序/非 FD/非除数
- 写入者排除全表见 §P6.6/P6.7 + 本轮：末循环写-读 race（C2V2Last 闭合后 mere 不动）
- **下一战役建议**：嫌疑收窄到"fixpipe 原子写在 m<64 几何下写出错误终值"本身——
  建议直接 dump 脏行完整 512 列累计器终值 + 除数，与宿主 fp64 重算的量化域期望
  （模拟 n(i) 尺度链）逐列对照，定位错误数值的代数形态（多算块/错尺度/错行混合）。

### 终态（kw-13 二进制，全部验证）

- MLA C04-C20：**15/16 ulp 确定性**（唯 C10）；C02 与 C10 并列永久遗留
- GQA：G1/G2 bit=True mere=0.0、C21 6.3e-6——与 kw-4 终态逐位同值
- kw-13 提交链：快照 v2 / 双读探针 / C2V2Last（1a2b3c4d5e... 见 git log）
