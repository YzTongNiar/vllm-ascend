...
template <typename T> __aicore__ inline T AlignUp(T num, T rnd)
{
    return (((rnd) == 0) ? 0 : (((num) + (rnd)-1) / (rnd) * (rnd)));
}

#if ((__CCE_AICORE__ == 310) || (defined __DAV_310R6__) || (__NPU_ARCH__ == 5102))
template <typename T> __aicore__ inline uint32_t GetBlockNum(uint32_t size)
{
                                     ...
                                     uint32_t mSplitSize)
{
    ...
    if (loadData2DParamsA.ifTranspose) {
        loadData2DParamsA.mStep =
            ((kSplitSize + 15) >> 4 << 4) >>
            4; // 以M*K矩阵为例,源矩阵M轴方向搬运长度(S1向上对齐分形(512B),16*16个f16->向上对齐16)，单位为16 element,取值范围：mStep属于[0,255]
                                  ...
                                  struct MMParam &param)
{
    if ((param.singleK + baseK - 1) / baseK > 1) {
        ...
        MatmulFull<A, B, C, baseM, baseN, baseK, AL, BL>(
            aL1Tensor, bL1Tensor, aL0BuffsDb, bL0BuffsDb, cL0Tensor, param);
    }
}
        ...
        if (rowSize == kSplitSize) {
            ...
            for (uint32_t loop = 0; loop < loopTimes; loop++) {
                LoadDataWithTranspose(
                    aL0Tensor[loop * l0Offset], aL1Tensor[loop * l1Offset], loadData2dTransposeParams);
            }
        } else {
