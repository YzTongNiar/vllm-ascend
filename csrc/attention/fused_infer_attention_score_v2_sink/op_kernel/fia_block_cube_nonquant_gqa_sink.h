...
/*!
 * \file fia_block_cube_nonquant_gqa_sink.h
 * \brief
 */
#ifndef FIA_BLOCK_CUBE_NONQUANT_GQA_SINK_H
#define FIA_BLOCK_CUBE_NONQUANT_GQA_SINK_H

#if __has_include("../common/op_kernel/array.h")
#include "array.h"
#include "axis.h"
 ...
 * 【Q2V2KP3 Tiling方案】
 * 1. Q常驻，V全载；
 * 2. 为Q分配2块L1 buffer，每块96KB(256*192); V分配2块L1 buffer，每块64KB(256*128)；K和P共享3块L1 buffer, 每块64KB(192*128 or 128*256)
 ...
 */
template <typename FIAT, typename Config = void> class FiaBlockCubeNonQuantGqa {
    struct DefaultCfg {
  ...
  private:
     ...
     */
    template <typename T1> static __aicore__ inline constexpr T1 Align(T1 s, T1 alignment)
    {
    ...
    template <typename T1> static __aicore__ inline constexpr size_t BlockAlign(size_t s)
    {
    ...
    template <typename T1> __aicore__ inline constexpr uint32_t GetC0Num()
    {
        if constexpr (IsSameType<T1, int4b_t>::value) {
            return 64;
        } else {
    ...
    __aicore__ inline void CopyKToL1(uint32_t dstBufId, RunInfo &info, uint32_t subNStart, uint32_t subNSize);
    template <CubeFormat Format>
    __aicore__ inline void CopyPToL1(uint32_t dstBufId,
                                     const RunInfo &info,
                                     uint32_t gmStride,
                                     uint32_t subMStart,
                                     uint32_t subMSize,
    ...
    template <uint32_t M_L1_SIZE, bool FOECE = false> __aicore__ inline void ResetLoad3DConfig();

    template <uint32_t M_L1_SPLIT_Size, typename T1>
    __aicore__ inline void LoadAToL0(uint32_t dstBufId,
                                     const LocalTensor<T1> &l1Tensor,
                                     uint32_t mL1Size,
                                     uint32_t subMStart,
                                     uint32_t subMSize,
                                     uint32_t subKStart,
                                     uint32_t subKSize);
    __aicore__ inline void LoadBTransposeToL0(uint32_t dstBufId,
                                              const LocalTensor<KV_T> &l1Tensor,
                                              uint32_t nL1Size,
                                              uint32_t subNStart,
                                              uint32_t subNSize,
                                              uint32_t subKStart,
                                              uint32_t subKSize);
    __aicore__ inline void LoadBToL0(uint32_t dstBufId,
                                     const LocalTensor<KV_T> &l1Tensor,
                                     uint32_t kL1Size,
                                     uint32_t subKStart,
                                     uint32_t subKSize,
    ...
    __aicore__ inline void FixpipeCToGM(GlobalTensor<MM_OUT_T> &mmResGm,
                                        uint32_t cL0BufId,
                                        uint32_t dstStride,
                                        uint32_t subMStart,
                                        uint32_t subMSize,
  ...
  private:
    ...
    static constexpr uint32_t L1_SINK_V_SIZE = 128 * 128;
    static constexpr uint32_t L1_KP_SIZE = 128 * 256; // 64KB. K: 128 * 192 or 128 * 256; P: 128 * 256
    static constexpr uint32_t L1_SINK_KP_SIZE = 128 * 192; // 对于P 只用到128*128， 对于K：用到128*192
    static constexpr uint32_t L1_K_SINK_SIZE = 128 * 128;
    ...
    static constexpr uint32_t L1_K_SINK_BUFCNT = 2;

    Array<LocalTensor<Q_T>, L1_Q_BUFCNT, L1_Q_SIZE> qL1Tensor;
    Array<LocalTensor<KV_T>, L1_V_BUFCNT, L1_SINK_V_SIZE> vL1Tensor;
    Array<LocalTensor<KV_T>, L1_KP_BUFCNT, L1_SINK_KP_SIZE> kpL1Tensor;
    Array<LocalTensor<KV_T>, L1_K_SINK_BUFCNT, L1_K_SINK_SIZE> kSinkL1Tensor;
...
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::UpdateKey(uint32_t bIdx)
{
    ...
    InitKeyGm(bIdx);
    uint64_t s2Size = FusedInferAttentionScoreV2SinkBaseKernel::SeqLenFromTensorList<KV_LAYOUT_T>(keyPtr, bIdx);
    keyGmTensor.gmTensor = keyGm;
    keyGmTensor.offsetCalculator.Init(
        0, constInfo.kvHeadNum, s2Size, constInfo.headDim, actualSeqLengthsGm, constInfo.actualLenDims);
}
...
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::UpdateValue(uint32_t bIdx)
{
    ...
    InitValueGm(bIdx);
    uint64_t s2Size = FusedInferAttentionScoreV2SinkBaseKernel::SeqLenFromTensorList<KV_LAYOUT_T>(valuePtr, bIdx);
    valueGmTensor.gmTensor = valueGm;
    valueGmTensor.offsetCalculator.Init(
        0, constInfo.kvHeadNum, s2Size, constInfo.headDim, actualSeqLengthsGm, constInfo.actualLenDims);
}

template <typename FIAT, typename Config>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::Init(__gm__ uint8_t *query,
                                                                   __gm__ uint8_t *key,
                                                                   __gm__ uint8_t *value,
                                                                   __gm__ uint8_t *pseShift,
                                                                   __gm__ uint8_t *attenMask,
                                                                   ...
                                                                   __gm__ uint8_t *softmaxLse)
{
    ...
    queryGm.SetGlobalBuffer((__gm__ Q_T *)query);
    {
        ...
        if constexpr (GmLayoutParams<Q_FORMAT>::CATEGORY == FormatCategory::GM_Q_OUT_BNGSD) {
            queryGmTensor.offsetCalculator.Init(constInfo.batchSize,
                                                constInfo.kvHeadNum,
                                                constInfo.gSize,
                                                constInfo.qSeqSize,
                                                qkTensorD,
            ...
            queryGmTensor.offsetCalculator.Init(
                constInfo.kvHeadNum, constInfo.gSize, qkTensorD, actualSeqLengthsGmQ, constInfo.actualLenQDims);
        }
    }

    if (likely(constInfo.keySinkNumber)) {
        keySinkGm.SetGlobalBuffer((__gm__ KV_T *)keySink);
        keyRopeSinkGm.SetGlobalBuffer((__gm__ KV_T *)keyRopeSink);
        valueSinkGm.SetGlobalBuffer((__gm__ KV_T *)valueSink);
    }
    ...
    if (constInfo.ropeSplitMode) {
        ...
        if constexpr (GmLayoutParams<Q_FORMAT>::CATEGORY == FormatCategory::GM_Q_OUT_BNGSD) {
            queryRopeGmTensor.offsetCalculator.Init(constInfo.batchSize,
                                                    constInfo.kvHeadNum,
                                                    constInfo.gSize,
                                                    constInfo.qSeqSize,
            ...
            queryRopeGmTensor.offsetCalculator.Init(constInfo.kvHeadNum,
                                                    constInfo.gSize,
                                                    constInfo.headDimRope,
                                                    actualSeqLengthsGmQ,
                                                    constInfo.actualLenQDims);
        ...
        if constexpr (PAGE_ATTENTION) {
            if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_PA_BNBD) {
                keyRopeGmTensor.offsetCalculator.Init(constInfo.kvHeadNum,
                                                      constInfo.kvCacheBlockSize,
                                                      constInfo.headDimRope,
                                                      blockTableGm,
                                                      constInfo.maxBlockNumPerBatch,
                                                      constInfo.keyRopeStride);
            ...
            if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_BNSD) {
                keyRopeGmTensor.offsetCalculator.Init(constInfo.batchSize,
                                                      constInfo.kvHeadNum,
                                                      constInfo.kvSeqSize,
                                                      constInfo.headDimRope,
                                                      actualSeqLengthsGm,
                ...
                keyRopeGmTensor.offsetCalculator.Init(
                    constInfo.kvHeadNum, constInfo.headDimRope, actualSeqLengthsGm, constInfo.actualLenDims);
            }
        }
    }
    ...
    if (constInfo.batchContinuous) {
        InitKeyGm(0);
        {
            ...
            if constexpr (PAGE_ATTENTION) {
                if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_PA_BNBD) {
                    keyGmTensor.offsetCalculator.Init(constInfo.kvHeadNum,
                                                      constInfo.kvCacheBlockSize,
                                                      qkTensorD,
                                                      blockTableGm,
                                                      constInfo.maxBlockNumPerBatch,
                ...
                if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_BNSD) {
                    keyGmTensor.offsetCalculator.Init(constInfo.batchSize,
                                                      constInfo.kvHeadNum,
                                                      constInfo.kvSeqSize,
                                                      qkTensorD,
                                                      actualSeqLengthsGm,
                    ...
                    keyGmTensor.offsetCalculator.Init(
                        constInfo.kvHeadNum, qkTensorD, actualSeqLengthsGm, constInfo.actualLenDims);
                }
            }
        }
        ...
        InitValueGm(0);
        {
            ...
            if constexpr (PAGE_ATTENTION) {
                if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_PA_BNBD) {
                    valueGmTensor.offsetCalculator.Init(constInfo.kvHeadNum,
                                                        constInfo.kvCacheBlockSize,
                                                        constInfo.headDim,
                                                        blockTableGm,
                                                        constInfo.maxBlockNumPerBatch,
                                                        constInfo.valueStride);
                } else if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_PA_NZ) {
                    uint32_t d0 = 32 / sizeof(KV_T);
                ...
                if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_BNSD) {
                    valueGmTensor.offsetCalculator.Init(constInfo.batchSize,
                                                        constInfo.kvHeadNum,
                                                        constInfo.kvSeqSize,
                                                        constInfo.headDim,
                                                        actualSeqLengthsGm,
                    ...
                    valueGmTensor.offsetCalculator.Init(
                        constInfo.kvHeadNum, constInfo.headDim, actualSeqLengthsGm, constInfo.actualLenDims);
                }
            }
        }
    }
...
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::InitBuffers(TPipe *pipe)
{
    ...
    pipe->InitBuffer(L1_V, L1_SINK_V_SIZE * L1_V_BUFCNT * sizeof(KV_T)); // L1_SINK_V_SIZE:128*128
    vL1Tensor.Init(L1_V.Get<KV_T>());
    pipe->InitBuffer(L1_KP, L1_SINK_KP_SIZE * L1_KP_BUFCNT * sizeof(KV_T)); // L1_SINK_KP_SIZE:128*192
    kpL1Tensor.Init(L1_KP.Get<KV_T>());
    pipe->InitBuffer(L1_K_SINK, L1_K_SINK_SIZE * L1_K_SINK_BUFCNT * sizeof(KV_T));
    kSinkL1Tensor.Init(L1_K_SINK.Get<KV_T>());

    // L0A [kw-3 诊断回退] A5 下也走原 TBuf L0A/L0B（KPP Buffer 版死锁待二分）
...
template <typename FIAT, typename Config> __aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::AllocEventID()
{
    for (uint32_t i = 0; i < L1_KP_BUFCNT; ++i) {
        SetFlag<HardEvent::MTE1_MTE2>(KP_EVENT0 + i);
    }
    SetFlag<HardEvent::MTE1_MTE2>(KP_EVENT0 + 3);
...
template <typename FIAT, typename Config> __aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::FreeEventID()
{
    for (uint32_t i = 0; i < L1_KP_BUFCNT; ++i) {
        WaitFlag<HardEvent::MTE1_MTE2>(KP_EVENT0 + i);
    }
    WaitFlag<HardEvent::MTE1_MTE2>(KP_EVENT0 + 3);
    for (uint32_t i = 0; i < L1_V_BUFCNT; ++i) {
...
template <typename FIAT, typename Config>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::CopyQToL1(uint32_t dstBufId,
                                                                        const RunInfo &info,
                                                                        uint32_t subMStart,
                                                                        uint32_t subMSize)
{
    auto qL1Tensor = this->qL1Tensor[dstBufId];
    auto subMSizeAlign = Align<uint32_t>(
        subMSize,
        (uint32_t)
...
template <typename FIAT, typename Config>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::CopyKToL1(uint32_t dstBufId,
                                                                        RunInfo &info,
                                                                        uint32_t subNStart,
                                                                        uint32_t subNSize)
{
    ...
    if (unlikely(constInfo.keySinkNumber && info.isSinkTensor)) {
        auto kSinkL1Tensor = this->kSinkL1Tensor[dstBufId];
        uint64_t offset = info.n2Idx * nopeDealSize;
        CopySingleMatrixNDToNZ(kSinkL1Tensor,
                                keySinkGm[offset],
                                subNSize,
                                nopeDealSize,
                                constInfo.kvHeadNum * nopeDealSize,
                                subNSizeAlign);
        if (constInfo.ropeSplitMode) {
            offset = info.n2Idx * constInfo.headDimRope;
            CopySingleMatrixNDToNZ(kSinkL1Tensor[subNSizeAlign * nopeDealSize],
                                    keyRopeSinkGm[offset],
                                    subNSize,
                                    constInfo.headDimRope,
...
template <typename FIAT, typename Config>
template <CubeFormat Format>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::CopyPToL1(uint32_t dstBufId,
                                                                        const RunInfo &info,
                                                                        uint32_t gmStride,
                                                                        uint32_t subMStart,
                                                                        uint32_t subMSize,
...
template <typename FIAT, typename Config>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::CopyVToL1(uint32_t dstBufId,
                                                                        const RunInfo &info,
                                                                        uint32_t subKStart,
                                                                        uint32_t subKSize)
{
    ...
    if (unlikely(info.isSinkTensor)) {
        uint64_t offset = info.n2Idx * constInfo.headDim;
        CopySingleMatrixNDToNZ(vL1Tensor,
                               valueSinkGm[offset],
                               subKSize,
                               (uint32_t)constInfo.headDim,
...
template <typename FIAT, typename Config>
template <uint32_t M_L1_SPLIT_Size, typename T1>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::LoadAToL0(uint32_t dstBufId,
                                                                        const LocalTensor<T1> &l1Tensor,
                                                                        uint32_t mL1Size,
                                                                        uint32_t subMStart,
                                                                        uint32_t subMSize,
...
template <typename FIAT, typename Config>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::LoadBTransposeToL0(uint32_t dstBufId,
                                                                                 const LocalTensor<KV_T> &l1Tensor,
                                                                                 uint32_t nL1Size,
                                                                                 uint32_t subNStart,
                                                                                 uint32_t subNSize,
                                                                                 ...
                                                                                 uint32_t subKSize)
{
    ...
    if (nL1Size == subNSize) {
        mm1LoadDataBTransposeToL0Params.repeatTimes =
            CeilDiv(subKSize, (uint32_t)BLOCK_CUBE) * CeilDiv(subNSize, GetC0Num<KV_T>());
        ...
        for (uint32_t i = 0; i < kLoops; i++) {
            LoadData(dstTensor[subNSize * i * BLOCK_CUBE],
                     srcTensor[nL1Size * i * GetC0Num<KV_T>()],
...
template <typename FIAT, typename Config>
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::LoadBToL0(uint32_t dstBufId,
                                                                        const LocalTensor<KV_T> &l1Tensor,
                                                                        uint32_t kL1Size,
                                                                        uint32_t subKStart,
                                                                        uint32_t subKSize,
                                                                        ...
                                                                        uint32_t subNSize)
{
    ...
    for (uint32_t i = 0; i < kLoops; i++) {
        LoadData(
            dstTensor[i * BLOCK_CUBE * subNSize], srcTensor[i * BLOCK_CUBE * GetC0Num<KV_T>()], mm2LoadDataBToL0Params);
    }
}
...
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::ComputeMm1(RunInfo &info)
{
    if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_BNSD) {
    ...
    uint32_t kpOrSinkL1BufId = info.isSinkTensor ? keySinkL1BufId : this->kpL1BufId;
    constexpr uint32_t M_SPLIT_SIZE = CFG::S2_BASICSIZE_IS_1024 ? 128 : 256;
    const uint32_t N_SPLIT_SIZE = 128;
    ...
    for (auto &nL1 : n.Split(N_SPLIT_SIZE)) {
        WaitFlag<HardEvent::MTE1_MTE2>(KP_EVENT0 + kpOrSinkL1BufId);

        // 第一处跳过检查：对整个mSize和当前nL1块进行稀疏边界检查
        if (unlikely(constInfo.isSkipSmallBlock && !constInfo.batchInvariant &&
                     FusedInferAttentionScoreV2SinkBaseKernel::IsSkipCal<LAYOUT_T>(info.gS1Idx,
                                                         info.actMBaseSize, // 整个m范围
                                                         info.s2Idx * constInfo.s2BaseSize + nL1.start,
                                                         nL1.sizeAct, // 当前nL1块
                                                         info,
                                                         constInfo))) {
            // 为下一次循环准备 SetFlag，避免死锁
            SetFlag<HardEvent::MTE1_MTE2>(KP_EVENT0 + kpOrSinkL1BufId);
            if (likely(!info.isSinkTensor)) {
                ++kpOrSinkL1BufId;
                if (kpOrSinkL1BufId >= L1_KP_BUFCNT) {
                    kpOrSinkL1BufId = 0;
                }
                this->kpL1BufId = kpOrSinkL1BufId;
            }
            continue;
        }
        uint32_t tmpBufId = info.isSinkTensor ? 0 : kpOrSinkL1BufId;
        CopyKToL1(tmpBufId, info, nL1.start, nL1.sizeAct);

        SetFlag<HardEvent::MTE2_MTE1>(KP_EVENT0 + kpOrSinkL1BufId);
        WaitFlag<HardEvent::MTE2_MTE1>(KP_EVENT0 + kpOrSinkL1BufId);

        int32_t mL1Id = -1;
        for (auto &mL1 : mSlices) {
            ++mL1Id;

            // 第二处跳过检查：对每个mL1小块和当前nL1块进行稀疏边界检查
            if (unlikely(constInfo.isSkipSmallBlock && !constInfo.batchInvariant &&
                         FusedInferAttentionScoreV2SinkBaseKernel::IsSkipCal<LAYOUT_T>(info.gS1Idx + mL1.start,
                                                             mL1.sizeAct, // 当前mL1块
                                                             info.s2Idx * constInfo.s2BaseSize + nL1.start,
                                                             nL1.sizeAct, // 当前nL1块
                                                             info,
            ...
            for (auto &mL0 : mL1.Split(M_L0_SPLIT_A5)) {
                for (auto &nL0 : nL0Slices) {
                    ...
                    // [kw-3 诊断回退] A5 下也走原 load3d 装载路径（KPP 版死锁待二分，
                    // 本轮只保留 D' 探针做 Q 路径裁决；详见 MIGRATION_NOTES.md P4 续）
                    for (auto &kL0 : kL0Slices) {
                        WaitFlag<HardEvent::M_MTE1>(L0AB_EVENT0 + this->abL0BufId);

                        LoadAToL0<M_SPLIT_SIZE>(this->abL0BufId,
                                                qL1Tensor[qBufId],
                                                mL1.AlignedSize(),
                                                mL0.start,
                                                mL0.AlignedSize(),
                                                ...
                                                kL0.AlignedSize());
                        if (unlikely(info.isSinkTensor)) {
                            LoadBTransposeToL0(this->abL0BufId,
                                               kSinkL1Tensor[0],
                                               nL1.AlignedSize(),
                                               nL0.start,
                                               nL0.AlignedSize(),
                        ...
                        Mmad(cL0Tensor[this->cL0BufId],
                             aL0Tensor[this->abL0BufId],
                             bL0Tensor[this->abL0BufId],
                             mmadParams);
                        if (likely(!isLastK)) {
                    ...
                    FixpipeCToGM<OutFormat>(
                        ...
                        (OutFormat == CubeFormat::NZ)
                            ? m.sizeAct
                            : info.actualSingleProcessSInnerSizeAlign, /* 输出ND时，mm1ResGm两行之间间隔元素个数按32对齐 */
                        mL1.start + mL0.start,
                        mL0.sizeAct,
                        nL1.start + nL0.start,
...
__aicore__ inline void FiaBlockCubeNonQuantGqa<FIAT, Config>::ComputeMm2(const RunInfo &info)
{
    ...
    // 原实现按 headDim=128 取 K 分块 128 (128×128=半区 exact-fit);
    // headDim=192 时 tile ×1.5 (8×3072=24,576 el > 16,384) 越过 L0B ping-pong 半区,
    // buf0 越界静默污染 buf1(错值) / buf1 越界写穿 L0B(0x26)。按容量推导 k 分块。
    const uint32_t kCap = (constInfo.headDim > 0) ? (L0B_PP_SIZE / constInfo.headDim) : (128U * 16U);
    const uint32_t K_SPLIT_SIZE = (kCap < 128U) ? (kCap & ~(BLOCK_CUBE - 1U)) : 128U;
    const uint32_t K_BASE = K_SPLIT_SIZE;
    ...
    for (auto &mL1 : m.Split(M_L1_SPLIT_MM2_A5)) {
        ...
        for (auto &kL1 : KL1Slices) {
            ...
            CopyPToL1<AFormat>(this->kpL1BufId,
                               info,
            ...
            // [kw-3 诊断回退] A5 下也走原 load3d 装载路径（同 mm1）
        ...
        FixpipeCToGM<OutFormat>(mmResGm,
                                ...
                                mL1.sizeAct,
                                0,
                                (OutFormat == CubeFormat::NZ) ? n.AlignedSize() : n.sizeAct);
        SetFlag<HardEvent::FIX_M>(L0C_EVENT0 + this->cL0BufId);
        this->cL0BufId = (this->cL0BufId + 1) % L0C_BUFCNT;
    }
#endif
