...
namespace AiInfraInferenceAttentionCommon {
...
template <typename Q_T,
          ...
          FIA_LAYOUT KV_LAYOUT_T = FIA_LAYOUT::BSH,
          const bool SOFTMAX_WITH_BRC = false,
          typename... Args>
struct FIAType {
    using queryType = Q_T;
    using kvType = KV_T;
...
struct ConstInfo {
    ...
    uint64_t qTSize = 0ULL;
    uint64_t kTSize = 0ULL;
    uint64_t headDim = 0;
    ...
    // Non-contiguous stride support for K/V/K_ROPE tensors
    uint64_t keyStride[5] = {0, 0, 0, 0, 0};
    uint64_t valueStride[5] = {0, 0, 0, 0, 0};
    uint64_t keyRopeStride[5] = {0, 0, 0, 0, 0};
...
GetGS1Idx(uint32_t gS1Idx, uint32_t &gIdx, uint32_t &s1Idx, ConstInfo &constInfo)
{
    // GS1
