...
/*!
 * \file fia_block_cube_nonquant_mla_sink.h
 * \brief use 7 buffer for matmul l1, better pipeline
 */
#ifndef FIA_BLOCK_CUBE_NONQUANT_MLA_SINK_H
#define FIA_BLOCK_CUBE_NONQUANT_MLA_SINK_H
...
using namespace AiInfraInferenceCommonFaBaseMatmul;
using namespace AiInfraInferenceAttentionCommon;

template <typename FIAT> class FiaBlockCubeNonQuantMla {
  public:
    ...
    __aicore__ inline void ComputeMm1(const AiInfraInferenceAttentionCommon::RunInfo &info);
    __aicore__ inline void ComputeMm2(const AiInfraInferenceAttentionCommon::RunInfo &info);
  ...
  protected:
    ...
    __aicore__ inline void ProcessMm1(const AiInfraInferenceAttentionCommon::RunInfo &info,
                                      const MSplitInfo mSplitInfo);
    __aicore__ inline void ProcessMm2(const AiInfraInferenceAttentionCommon::RunInfo &info,
                                      const MSplitInfo mSplitInfo);
    __aicore__ inline void UpdateKey(GlobalTensor<KV_T> keyGm);
    __aicore__ inline void UpdateValue(GlobalTensor<KV_T> valueGm);
    __aicore__ inline void CopyGmToL1(LocalTensor<KV_T> &l1Tensor,
                                      GlobalTensor<KV_T> &gmSrcTensor,
                                      uint32_t srcN,
                                      uint32_t srcD,
                                      uint32_t srcDstride);
    __aicore__ inline void CopyInMm2AToL1(LocalTensor<KV_T> &aL1Tensor,
                                          const AiInfraInferenceAttentionCommon::RunInfo &info,
                                          uint32_t mSeqIdx,
                                          uint32_t subMSizeAct,
                                          uint32_t nSize,
                                          uint32_t nOffset);
    // __aicore__ inline void CopySingleMatrixNDToNZ(LocalTensor<KV_T> &l1Tensor, const GlobalTensor<KV_T> &gmTensor,
    //                                     uint32_t nValue, uint32_t dValue, uint32_t srcDValue, uint32_t
    //                                     dstNzC0Stride);
  ...
  private:
    ...
    AiInfraInferenceCommonFaBaseMatmul::BufferManager<
        AiInfraInferenceCommonFaBaseMatmul::BufferType::L1> l1BufferManager;
    AiInfraInferenceCommonFaBaseMatmul::BuffersPolicy3buff<
        AiInfraInferenceCommonFaBaseMatmul::BufferType::L1> l1KV3Buffers;
    AiInfraInferenceCommonFaBaseMatmul::Matrix2x2BufferPolicy<
        AiInfraInferenceCommonFaBaseMatmul::BufferType::L1> l1QP4Buffers;
#endif

    AiInfraInferenceCommonFaBaseMatmul::BufferManager<
        AiInfraInferenceCommonFaBaseMatmul::BufferType::L0A> l0aBufferManager;
    AiInfraInferenceCommonFaBaseMatmul::BufferManager<
        AiInfraInferenceCommonFaBaseMatmul::BufferType::L0B> l0bBufferManager;
    // L0A
    AiInfraInferenceCommonFaBaseMatmul::BuffersPolicyDB<
        AiInfraInferenceCommonFaBaseMatmul::BufferType::L0A> mmL0ABuffers;
    // L0B
    AiInfraInferenceCommonFaBaseMatmul::BuffersPolicyDB<
        AiInfraInferenceCommonFaBaseMatmul::BufferType::L0B> mmL0BBuffers;
...
__aicore__ inline void FiaBlockCubeNonQuantMla<FIAT>::Init(__gm__ uint8_t *query,
                                                           __gm__ uint8_t *key,
                                                           __gm__ uint8_t *value,
                                                           __gm__ uint8_t *pseShift,
                                                           __gm__ uint8_t *attenMask,
                                                           ...
                                                           __gm__ uint8_t *softmaxLse)
{
    ...
    if constexpr (GmLayoutParams<Q_FORMAT>::CATEGORY == FormatCategory::GM_Q_OUT_BNGSD) {
        ...
        queryRopeGmTensor.offsetCalculator.Init(
            constInfo.kvHeadNum, constInfo.gSize, constInfo.headDimRope, actualSeqLengthsGmQ, constInfo.actualLenQDims);
    }
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuantMla<FIAT>::CopyGmToL1(
    LocalTensor<KV_T> &l1Tensor, GlobalTensor<KV_T> &gmSrcTensor, uint32_t srcN, uint32_t srcD, uint32_t srcDstride)
{
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuantMla<FIAT>::CopyInMm2AToL1(
...
template <typename FIAT> __aicore__ inline void FiaBlockCubeNonQuantMla<FIAT>::ComputeMm2(const RunInfo &info)
{
    if (info.isChangeBatch) {
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuantMla<FIAT>::ProcessMm1(const AiInfraInferenceAttentionCommon::RunInfo &info,
                                                                 const MSplitInfo mSplitInfo)
{
    // 最外层还需要一层m的循环
...
#ifdef BASE_MM
    AiInfraInferenceCommonFaBaseMatmul::Buffer<AiInfraInferenceCommonFaBaseMatmul::BufferType::L1> mm1B;
    AiInfraInferenceCommonFaBaseMatmul::Buffer<AiInfraInferenceCommonFaBaseMatmul::BufferType::L1> mm1A;
    ...
    for (uint32_t nL1 = 0; nL1 < nL1Loops; nL1++) { // L1切n, 512/128
        ...
        for (uint32_t kL1 = 0; kL1 < kL1Loops; kL1++) { // L1切k, 576/288, 这里不考虑d泛化
            ...
            if (kL1 == 0) {
                ...
                // sink作为独立tensor传入时，要单独拷贝
                if ((gmCoord.s2Idx) || (!constInfo.keySinkNumber)) {
                    if (PAGE_ATTENTION) {
                        gmCoord.s2Idx =
                            (constInfo.keySinkNumber) ? (gmCoord.s2Idx - constInfo.keySinkNumber) : gmCoord.s2Idx;
                        gmCoordKRope.s2Idx = (constInfo.keySinkNumber) ? (gmCoordKRope.s2Idx - constInfo.keySinkNumber)
                                                                       : gmCoordKRope.s2Idx;
                        copyKKropePAGmToL1(
                            dstTensor, dstRopeTensor, keyGmTensor, keyRopeGmTensor, gmCoord, gmCoordKRope);
                    } else {
                        gmCoord.s2Idx =
                            (constInfo.keySinkNumber) ? (gmCoord.s2Idx - constInfo.keySinkNumber) : gmCoord.s2Idx;
                        gmCoordKRope.s2Idx = (constInfo.keySinkNumber) ? (gmCoordKRope.s2Idx - constInfo.keySinkNumber)
                    ...
                    uint64_t offset = info.n2Idx * constInfo.headDim; // s2方向的偏移为0
                    ...
                    offset = info.n2Idx * constInfo.headDimRope; // s2方向的偏移为0
                    CopySingleMatrixNDToNZ(dstRopeTensor.tensor,
                                           keyRopeSinkGm[offset],
                                           gmCoordKRope.s2DealSize,
                                           gmCoordKRope.dDealSize,
                                           constInfo.kvHeadNum * constInfo.headDimRope,
                                           dstRopeTensor.rowCount);
                }
            } else {
                ...
                if ((gmCoord.s2Idx) || (!constInfo.keySinkNumber)) {
                    ...
                    uint64_t offset = info.n2Idx * constInfo.headDimRope + kL1 * 32U; // s2方向的偏移为0
                    CopySingleMatrixNDToNZ(dstRopeTensor.tensor,
                                           keyRopeSinkGm[offset],
                                           gmCoordKRope.s2DealSize,
                                           gmCoordKRope.dDealSize,
                    ...
                    offset = info.n2Idx * constInfo.headDim + kL1 * 256U; // s2方向的偏移为0
                    CopySingleMatrixNDToNZ(dstTensor.tensor,
                                           keySinkGm[offset],
                                           gmCoord.s2DealSize,
                                           gmCoord.dDealSize,
            ...
            for (uint32_t mL1 = 0; mL1 < mL1Loops; mL1++) {
...
#endif
                    if (kL1 == 0) {
                        FaL1Tensor<Q_T, L1Format::NZ> dstTensor{.tensor = aL1Tensor, .rowCount = Align(mL1Size, 16U)};
...
#ifdef BASE_MM
                AiInfraInferenceCommonFaBaseMatmul::MMParam param;
                ...
                param.unitFlag = (kL1 == kL1Loops - 1)
                                    ? AiInfraInferenceCommonFaBaseMatmul::UNITFLAG_EN_OUTER_LAST
                                    : AiInfraInferenceCommonFaBaseMatmul::UNITFLAG_ENABLE;
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuantMla<FIAT>::ProcessMm2(const AiInfraInferenceAttentionCommon::RunInfo &info,
                                                                 const MSplitInfo mSplitInfo)
{
    uint32_t mSize = mSplitInfo.nBufferDealM;
        ...
        for (uint32_t k1 = 0; k1 < kL1Loops; k1++) { // k切L1, 这里套了一层l0来操作
            ...
            for (uint32_t kL1 = kOffset; kL1 < kL0Loops + kOffset; kL1++) { // 128 循环搬pa
                ...
                if ((gmCoord.s2Idx) || (!constInfo.keySinkNumber)) {
                    ...
                    uint64_t offset = info.n2Idx * constInfo.headDim + nL1 * N_L1_SPLIT_SIZE;
                    CopySingleMatrixNDToNZ(dstTensor.tensor,
                                           valueSinkGm[offset],
                                           gmCoord.s2DealSize,
                                           gmCoord.dDealSize,
            ...
            for (uint32_t mL1 = 0; mL1 < mL1Loops; mL1++) {
                ...
                if (nL1 == 0) {
                    ...
                    CopyInMm2AToL1(
                        aL1Tensor, info, mSplitInfo.nBufferStartM + mL1 * M_L1_SPLIT_SIZE, mL1Size, kL1Size, 256 * k1);
                ...
                if (k1 == (kL1Loops - 1)) {
                    ...
                    uint64_t fixpipeLoopIdx = constInfo.batchInvariant ? info.loop : info.bn2IdxInCurCore;
                    Fixpipe(
