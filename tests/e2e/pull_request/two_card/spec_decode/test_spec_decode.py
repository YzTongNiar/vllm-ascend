...
def test_eagle3_sliding_window():
    ...
    filler = (
        "This is a padding sentence for testing sliding window "
        "draft attention. " * 80
    )
    long_prompt = {
        "role": "user",
    ...
    assert prompt_tokens > draft_window_size, (
        f"Prompt ({prompt_tokens} tokens) must exceed "
        f"draft_window_size ({draft_window_size})"
    )
