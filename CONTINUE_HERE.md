...
> 更新：2026-09-03（kw-10 收工时点）。本文档是另一台机器上恢复全部工作的单一入口。
...
**代码终态**：分支 `feat/fia-v2-sink-a5` @ `cb79e16d9`（= kw-9a + kw-9b + kw-10 + bt 钳位）
...
**遗留清单**（按优先级）：
1. C10 残差（MLA G≤2 小 M 类）：机制已框至 V1/V2 AIV 侧小 m 路径或 nupdate 除数链，
   探针路线 = MIGRATION_NOTES §P6.5（dump aMlaSumUb/tmpSumUb 与 V2 读值，metadata[601/602] 选择器）
