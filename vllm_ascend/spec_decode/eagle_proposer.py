...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def _propose(
        ...
        if self.pcp_size * self.dcp_size > 1:
            ...
            # DEBUG: DCP long_seq_args in _propose
            if self.dcp_size > 1:
                print(f"[DEBUG][EagleProposer][dcp_rank={self.dcp_rank}] _propose long_seq: "
                      f"query_lens_d={query_lens_d[:min(4,query_lens_d.shape[0])].cpu().tolist() if query_lens_d is not None else None}... "
                      f"ori_token_indices_to_sample={ori_token_indices_to_sample[:min(4,ori_token_indices_to_sample.shape[0])].cpu().tolist() if ori_token_indices_to_sample is not None else None}...")
    ...
    def _run_merged_draft(
        ...
        # DEBUG: draft tokens from first pass
        if self.dcp_size > 1 and not (self.pcp_size * self.dcp_size > 1 and is_prefill):
            print(f"[DEBUG][EagleProposer][dcp_rank={self.dcp_rank}] _run_merged_draft first_pass: "
                  f"batch_size={batch_size} draft_token_ids={draft_token_ids[:min(8,batch_size)].cpu().tolist()}")
        ...
        for draft_step in range(self.num_speculative_tokens - 1):
            ...
            # DEBUG: draft tokens from subsequent steps
            if self.dcp_size > 1:
                print(f"[DEBUG][EagleProposer][dcp_rank={self.dcp_rank}] _run_merged_draft step={draft_step+1}: "
                      f"batch_size={batch_size} positions={positions[:min(4,batch_size)].cpu().tolist()}... "
                      f"draft_token_ids={draft_token_ids[:min(8,batch_size)].cpu().tolist()}")
    ...
    def set_inputs_first_pass(
        ...
        if not self.needs_extra_input_slots:
            ...
            # DEBUG: draft model first-pass inputs under DCP
            if self.dcp_size > 1:
                dcp_rank = self.runner.dcp_rank if self.runner else -1
                print(f"[DEBUG][EagleProposer][dcp_rank={dcp_rank}] set_inputs_first_pass: "
                      f"num_tokens={num_tokens} batch_size={cad.batch_size() if hasattr(cad,'batch_size') else '?'} "
                      f"target_positions={target_positions[:min(8,num_tokens)].cpu().tolist()} "
                      f"target_token_ids={target_token_ids[:min(8,num_tokens)].cpu().tolist()}")
    ...
    def attn_update_stack_num_spec_norm(
        ...
        if self.pcp_size * self.dcp_size > 1:
            ...
            common_attn_metadata.slot_mapping = self.slot_mapping_group[draft_step]
            # DEBUG: DCP draft slot mapping update
            if self.dcp_size > 1 and batch_size > 0:
                num_neg = int((slot_mapping == -1).sum())
                print(f"[DEBUG][EagleProposer][dcp_rank={self.dcp_rank}] attn_update step={draft_step}: "
                      f"batch_size={batch_size} slot_indices={slot_indices[:min(4,batch_size)].cpu().tolist()}... "
                      f"neg_slots={num_neg}/{slot_mapping.shape[0]} "
                      f"cp_seq_len={cp_seq_len[:min(4,batch_size)].cpu().tolist()}... "
                      f"ori_seq_len={ori_seq_len[:min(4,batch_size)].cpu().tolist() if ori_seq_len is not None else None}")
        else:
