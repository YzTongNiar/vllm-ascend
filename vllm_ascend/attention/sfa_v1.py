...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def _handle_o_proj_weight_switch_and_forward(
        ...
        """
        # Perform all-to-all communication on o_proj input activations.
        # This handles both prefill (should_shard_weight=True) and decode
        # (should_shard_weight=False) scenarios.
        if should_shard_weight and o_proj_full_handle is not None:
            # If a weight all-gather was initiated (e.g., prefill/merged-draft
            # path), wait for it to complete. The gathered weight is not used
            # directly — we use the all-to-all activation approach instead.
        ...
        # native
            ...
            if self.enable_dsa_cp:
                ...
                if self.enable_dsa_cp_with_layer_shard:
                    ...
                    pool, o_proj_full_handle = all_gather_async(
                        self.o_proj_tp_weight, get_tp_group()
                    )
