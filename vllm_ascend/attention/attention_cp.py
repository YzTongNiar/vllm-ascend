...
class AscendAttentionCPImpl(AscendAttentionBackendImpl):
    ...
    def _attention_with_nomask_and_mask(self, q: torch.Tensor,
        ...
        if k_nomask is not None:
            output, attn_lse = self._npu_attn_out_lse_update(attn_lse_mask,
                                        attn_lse_nomask,
                                        attn_out_mask,
                                        attn_out_nomask) 
    ...
    def _npu_attn_out_lse_update(self, attn_lse_mask, attn_lse_nomask,
        ...
        update_type = 0
        output, attn_lse_updated = torch_npu.npu_attention_update(attn_lse, attn_output,
                                                   update_type)
    ...
    def _update_chunk_attn_out_lse(self, prefix_chunk_output,
        ...
        prefix_output, prefix_lse = self._npu_attn_out_lse_update(attn_lse_allgather[1],
            attn_lse_allgather[0],
            attn_out_allgather[1],
            attn_out_allgather[0])
